round(block.region.x1)},${Math.round(block.region.y1)}) to (${Math.round(block.region.x2)},${Math.round(block.region.y2)})` :
            'No region set';
...
document.addEventListener('DOMContentLoaded', () => {
    const addUrlBtn = document.createElement('button');addUrlBtn.className = 'btn btn-outline-secondary';
